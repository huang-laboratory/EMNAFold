...
class PDBNABaseDataset(Dataset):
    ...
    def _process_csv_row(self, processed_file_path):
        ...
        chain_feats, na_chain_feats = (
            {
                ...
                "chain_index": torch.tensor(processed_feats["atom_chain_indices"]).long(),
            },
            {},
        )
        ...
        if na_inputs_present:
            na_chain_feats = {
                ...
                # MOD
                "residue_index": chain_feats["residue_index"][processed_feats["is_na_residue_mask"]],
                "chain_index": chain_feats["chain_index"][processed_feats["is_na_residue_mask"]].long(),
            }
        ...
        # MOD
        final_feats["chain_index"] = torch.tensor(processed_feats['atom_chain_indices']).int()
        final_feats["residue_index"] = torch.tensor(processed_feats['residue_index']).int()

        #print(final_feats["chain_index"])
        #print(final_feats["residue_index"])
    ...
    # TL
    @fn.lru_cache(maxsize=100)
    def _process_csv_row_secstr(self, secstr_file_path):
        data = du.read_pkl(secstr_file_path)
        single, pair, block, single_sse = data['single'], data['pair'], data['block'], data['single_sse']
        secstr_feats = {
            'single_secstr': single,
            'pair_secstr': pair,
            'pair_block': block,
    ...
    # TL
    @fn.lru_cache(maxsize=100)
    def _process_csv_row_pts(self, pts_path):
        data = du.read_pkl(pts_path)
        trans_pts = data
        pts_feats = {
            'trans_pts': trans_pts,
        }
    ...
    def __getitem__(self, idx):
        ...
        # MOD extract item csv_raw, something like: pdb_name,processed_path,raw_path,num_chains,quaternary_category,num_protein_chains,num_na_chains,seq_len,na_seq_len,modeled_seq_len,modeled_protein_seq_len,modeled_na_seq_len,coil_percent,helix_percent,strand_percent,radius_gyration
        csv_row = self.csv.iloc[example_idx]

        # MOD get .pkl file and process .pkl file
        # MOD add chain idx and res idx
        ...
        # MOD padding for different lengths
        final_feats = du.pad_feats(final_feats, csv_row["modeled_na_seq_len"])

        # MOD add secstr
        secstr_file_path = csv_row["secstr_path"]
        secstr_feats = self._process_csv_row_secstr(secstr_file_path)
        secstr_feats = tree.map_structure(
            lambda x: x if torch.is_tensor(x) else torch.tensor(x), secstr_feats
        )
        secstr_feats = du.pad_feats(secstr_feats, csv_row["modeled_na_seq_len"])

        # Update
        final_feats.update(secstr_feats)
        ...
        # MOD add em points
        pts_path = csv_row["pts_path"]
        pts_feats = self._process_csv_row_pts(pts_path)
        pts_feats = tree.map_structure(
            lambda x: x if torch.is_tensor(x) else torch.tensor(x), pts_feats
        )

        final_feats.update(pts_feats)

        # MOD convert to float32
