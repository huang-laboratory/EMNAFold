...
from em3na.na_utils.data import (
    protein_constants,
...
MACROMOLECULE_OUTPUTS_TYPE = Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Dict[str, Any]]
...
def parse_chain_for_molecule_type_validity(
    ...
    # MOD
    X, C, S, I, metadata = parsing.structure_to_XCS(
        structure=structure,
    ...
    is_valid_molecule_type = all(
        [len(X), len(C), len(S), len(I), all([len(v) for v in metadata.values()])]
    )

    # MOD use raw chain index (integers)
...
def process_chain_pdb(
    ...
    # MOD
    X, C, S, I, metadata = parse_chain_for_molecule_type_validity(
        structure=structure,
...
def macromolecule_outputs_to_dict(outputs: MACROMOLECULE_OUTPUTS_TYPE) -> Dict[str, Any]:
    ...
    outputs_dict = {
        ...
        # MOD
        "residue_index": outputs[3].numpy(),

        "atom_mask": outputs[4]["atom_mask"].numpy(),

        "atom_chain_indices": outputs[4]["chain_indices"].numpy(),

        "atom_deoxy": outputs[4]["deoxy"].numpy(),
        "atom_b_factors": outputs[4]["b_factors"].numpy(),
        "molecule_type_encoding": outputs[4]["molecule_type_encoding"].numpy(),
    }
    return outputs_dict
