...
from em3na.na_utils.data import complex_constants, nucleotide_constants, protein_constants
...
def complex_to_pdb(complex: Complex, model=1, add_end=True, molecule_type_to_write=-1) -> str:
    ...
    # TODO use atom mask from valid atoms
    ...
    # MOD restype condensed
    # A  C  G  T  U
    # 21 22 23 24 25

    # MOD restype not condensed
    # DA DC DG DT  A  G  C  U
    # 21 22 23 24 25 26 27 28

    assert ((21 <= restype) & (restype <= 28)).all(), "Invalid nucleotide type"
    ...
    for i in range(restype.shape[0]):
        ...
        #print(len(atom_positions[i]))
        #print(len(atom_mask[i]))

        for atom_name, pos, mask, b_factor in zip(
            atom_types, atom_positions[i], atom_mask[i], b_factors[i]
        ):

            #print(atom_name, mask, pos)
