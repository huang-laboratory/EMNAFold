...
from em3na.modules.attention_x import NodeAttention, EdgeAttention, EdgeTransition
from em3na.modules.ipa_pytorch_x import InvariantPointAttention, IPATransition, BackboneUpdate
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        node, edge, affines = model(
            node=torch.randn(10, 256).cuda(),
            edge=torch.randn(10, 10, 128).cuda(),
            affines=torch.randn(10, 3, 4).cuda(),
            node_pos=torch.randn(10, 3).cuda(),
            edge_pos=torch.randn(10, 10, 3).cuda(),
            mask=torch.ones(10).cuda(),
            use_checkpoint=False,
        )
        t1 = time.time()
