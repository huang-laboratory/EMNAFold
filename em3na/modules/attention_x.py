...
from em3na.modules.rope import node_rope, edge_rope
...
class NodeAttention(nn.Module):
    ...
    def forward(self, node, edge, bias, node_pos, mask=None):
        ...
        out = torch.einsum('... i j h, ... j h d -> ... i h d', attn, v)
...
class BiasedAxialAttention(nn.Module):
    ...
    def forward(self, edge, bias, edge_pos, edge_mask=None):
        ...
        q = self.q(edge)
        k = self.k(edge)
        v = self.v(edge) # (l, l, h, d)
        q, k = edge_rope(q, k, edge_pos)

        bias = self.b(bias) # (l, l, h)

        gate = torch.sigmoid(self.g(edge)) # (l, l, h * d)

        q = q * self.attn_scale
        k = k / L

        attn = torch.einsum('... n i h k, ... n j h k -> ... i j h', q, k) # tied attention
        attn = attn + bias # apply bias
        attn = nn.functional.softmax(attn, dim=-2) # (..., l, l, h)

        # TODO add mask

        # add mask
        edge_mask = self.inf * (edge_mask - 1) # (..., l, l)
        attn = attn + edge_mask.unsqueeze(-1)

        # get output
        out = torch.einsum('... i j h, ... n j h d -> ... n i h d', attn, v)
        out = gate * out
