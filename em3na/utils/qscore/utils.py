...
def get_reference_gaussian_params(map) -> Tuple[float, float]:
...
def interpolate_grid_at_points(points: np.ndarray, map) -> np.ndarray:
    ...
    # Boundary check
    p = np.clip(p, 0.0, np.asarray( map.grid.shape ) - 1)
