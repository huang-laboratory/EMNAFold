...
import numpy as np
from Bio.PDB import PDBParser, MMCIFParser, MMCIFIO, PDBIO
...
def get_protein_from_file_path(file_path: str, chain_id: str = None) -> Protein:
    ...
    for chain in model:
        ...
        for res in chain:
            ...
            if resname not in restype_3_to_index:
                continue
            res_shortname = restype_3to1[resname]
            ...
            for atom in res:
                ...
                if atomname not in atom_types:
                    continue
                pos[atom_order[atomname]] = atom.coord
                posc[restype3_to_atoms_index[resname][atomname]] = atom.coord
                mask[atom_order[atomname]] = 1.0
                maskc[restype3_to_atoms_index[resname][atomname]] = 1.0
                res_b_factors[atom_order[atomname]] = atom.bfactor
            if np.sum(mask) < 0.5:
