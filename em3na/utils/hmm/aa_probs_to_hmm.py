...
from em3na.utils.misc_utils import assertion_check
from em3na.utils.residue_constants import (
...
def aa_log_probs_to_hmm_file(
    ...
    with open(output_path, "w") as file_handle:
        ...
        if alphabet_type == "amino":
            # The top-20 amino-acid log probs
...
def aa_logits_to_hmm(
    ...
    if alphabet_type == "amino":
        ...
        if alphabet_type == "RNA":
            ...
            c = aa_logits[..., 24:].max(axis=-1, keepdims=True)
            m = aa_logits[..., 24:].min(axis=-1, keepdims=True)
            exp_logits = np.exp(aa_logits - c)
            exp_logits_gather = np.zeros_like(exp_logits) + np.exp(m - c)

            # No merge purine-pyrimidine logits
            ...
            # What follows is a custom implementation of log_softmax
            c = aa_logits[..., 20:24].max(axis=-1, keepdims=True)
            m = aa_logits[..., 20:24].min(axis=-1, keepdims=True)
            exp_logits = np.exp(aa_logits - c)
            exp_logits_gather = np.zeros_like(exp_logits) + np.exp(m - c)

            # No merge purine-pyrimidine logits
            exp_logits_gather[..., restype_3_to_index["DA"]] = exp_logits[
                ...,
                [
                    restype_3_to_index["DA"],
                ],
            ].sum(axis=-1)
            exp_logits_gather[..., restype_3_to_index["DC"]] = exp_logits[
                ...,
                [
                    restype_3_to_index["DC"],
                ],
            ].sum(axis=-1)
            exp_logits_gather[..., restype_3_to_index["DG"]] = exp_logits[
                ...,
                [
                    restype_3_to_index["DG"],
                ],
            ].sum(axis=-1)
            exp_logits_gather[..., restype_3_to_index["DT"]] = exp_logits[
                ...,
                [
    ...
    tmp_path = os.path.join(base_dir, f"aa_temp.hmm")
