...
# Only inference ca/c4
def inference_c4(dir_map, contour, dir_model, dir_out, data_params, test_params, device):
    print(f"# Select map contour at {contour:.6f}", flush=True)
    print(f"# Running on device {device}")
    ...
    mask = np.where(map <= contour, 0, 1).astype(np.int8)
    out = mask * map_pred[0]
    dir_map_out = os.path.join(dir_out, "c4.mrc")
    write_map(dir_map_out, out.astype(np.float32), voxel_size, origin=origin)
    print("# Write map to {}".format(dir_map_out), flush=True)
...
def inference_aa(dir_map, contour, dir_model, dir_out, data_params, test_params, device):
    ...
    map_pred = np.argmax(map_pred, axis=0)
    dir_map_out = pjoin(dir_out, "aa.mrc")
    write_map(dir_map_out, map_pred.astype(np.float32), voxel_size, origin=origin)

    print("# Write aa mrc file to {}".format(dir_map_out), flush=True)
...
def main(args):
    ...
    if not os.path.exists(pjoin(dir_model, "stage1")):
        raise Exception("Cannot find model weights for stage1")
    if not os.path.exists(pjoin(dir_model, "stage2_prob")):
        raise Exception("Cannot find model weights for stage2_prob")
    ...
    # Run mol segmentation
    print("# Start inference segmentation")
    inference_segmentation(
        dir_map, 
        contour, 
        pjoin(dir_model, "stage1"), 
        dir_out, 
        data_params, 
        test_params, 
        device=device, 
    ...
    # Run main chain
    print("# Start inference C4'", flush=True)
    inference_c4(
        pjoin(dir_out, "na.mrc"), 
        contour, 
        pjoin(dir_model, "stage2_prob"), 
        dir_out, 
        data_params, 
        test_params, 
        device=device, 
...
def add_args(parser):
    ...
    parser.add_argument("--model", "-m", type=str, help="Directory to deep learning models", default=pjoin(script_dir, "..", "weightsx"))
