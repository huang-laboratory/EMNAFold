...
#ifndef PDBParserx_CPP
#define PDBParserx_CPP 1

#include "PDBParserx.h"
#include "pstream.h"
#include <string>
...
// Data
ModelUnit read_pdb_structure_data(const char* filename){
    ModelUnit pep;
    ChainUnit chain;
    ResidueUnit residue;
    AtomUnit atom;
    ...
    auto atom_name_extended = std::unordered_map<std::string, std::string>{
        // RNAs
        {"'C3''" , " C3'"},
        {"'C4''" , " C4'"},
        {"'O4''" , " O4'"},
    ...
    auto res_name_extended = std::unordered_map<std::string, std::string>{
        {"A",  "  A"},
        {"C",  "  C"},
        {"G",  "  G"},
        {"U",  "  U"},
    ...
    while (std::getline(fin, line)){
        // ATOM C4' 1 151.165 254.055 157.543
        // RESIDUE A 1
        // CHAIN A 1
        // MODEL A 1
        if (line.substr(0, 4) == "ATOM"){
            ...
            iss >> header >> atom_name >> atom_idx >> x >> y >> z;

            atom.name = atom_name_extended[atom_name];
            atom.xyz[0] = x;
            atom.xyz[1] = y;
            atom.xyz[2] = z;
        ...
        }else if (line.substr(0, 7) == "RESIDUE"){
            std::istringstream iss(line);
            std::string header;
            std::string res_name;
            int res_idx;

            iss >> header >> res_name >> res_idx;

            residue.resn = res_name_extended[res_name];
            residue.resi = res_idx;
            residue.icode = ' ';
            residue.het = false;
            ...
            iss >> header >> chain_name >> chain_idx;

            chain.chainID = chain_name[0];
            chain.chainID_full = chain_name;
