...
void CSSR::run() {
    ...
    ModelUnit pdb_entry = read_pdb_structure_data(input.c_str());
    ...
    for (size_t c=0;c<pdb_entry.chains.size();c++)
        for (size_t r=0;r<pdb_entry.chains[c].residues.size();r++)
            if (pdb_entry.chains[c].residues[r].het==false){
                ...
                //std::cout << pdb_entry.chains[c].residues[r].resn << std::endl;
            }
    ...
    // mod by TL
    //for(int i = 0; i < res_str_vec.size(); ++i){
    //    cout << res_str_vec[i] << endl;
    //}

    //for(int i = 0; i < filtered_bp_vec.size(); ++i){
    //    cout << filtered_bp_vec[i] << endl;
    //}

    //for(int i = 0; i < bp_vec.size(); ++i){
    //    string s = "";
    //    for(int k = 0; k < bp_vec[i].second.size(); ++k){
    //        s += bp_vec[i].second[k];
    //    }
    //    cout << bp_vec[i].first << ' ' << s << endl;
    //}
    //exit(0);
